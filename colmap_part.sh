...
colmap mapper \
  --database_path "$DB_PATH" \
  --image_path "$MERGED_DIR" \
  --output_path "/data02/zhangwei/colmap/jianzhu/sparse/run_fast" \
  --Mapper.num_threads 8 \
  --Mapper.ba_global_max_num_iterations 10 \  # 设置一个较小的全局优化迭代次数
  --Mapper.ba_local_num_images 20 \  # 每次局部优化使用的图像数
  --Mapper.ba_local_max_num_iterations 20 \  # 局部优化的最大迭代次数
  --Mapper.init_min_num_inliers 80 \  # 初始匹配最小内点数
  --Mapper.abs_pose_min_num_inliers 25 \  # 最小绝对位姿内点数
  --Mapper.tri_ignore_two_view_tracks 1 \  # 忽略两视角追踪的点
  --Mapper.filter_max_reproj_error 4 \  # 最大重投影误差
  --Mapper.filter_min_tri_angle 1.5  # 最小三角形角度
colmap bundle_adjuster \
  --input_path "/data02/zhangwei/colmap/jianzhu/sparse/run_fast/0" \  # 输入路径
  --output_path "/data02/zhangwei/colmap/jianzhu/sparse/run_fast_ba/0" \  # 输出路径
  --BundleAdjustment.refine_focal_length 1 \  # 优化焦距
  --BundleAdjustment.refine_principal_point 0 \  # 不优化主点
  --BundleAdjustment.refine_extra_params 1 \  # 优化其他额外参数
